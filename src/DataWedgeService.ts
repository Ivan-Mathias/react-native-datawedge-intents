...
interface RNDataWedgeModule {
  sendBroadcastWithExtras(extras: any): void
  registerBroadcastReceiver(filter: any): void
}

const RNDataWedgeIntents = NativeModules.DataWedgeIntents as RNDataWedgeModule
...
const DataWedgeService = {
  initialize: ({ packageName, profileName, disableKeystroke = true }: DataWedgeConfig) => {
   ...
   */
  setIllumination: (flashOn: boolean) => {
 ...
 */
function sendCommand(command: string, parameter: any) {
  if (Platform.OS !== 'android') return
